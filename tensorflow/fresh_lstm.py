...
class get_better_data(object):
    ...
    def get(self, file_name, vector_length=10):
        ...
        return string_addresses, integer_addresses, np.asarray(features), np.asarray(still_addresses), \
               np.asarray(feature_labels), labels, labels_vector
...
def Main():
    ...
    _, int_train_features, train_features, still_train_features, _, _, train_labels = data.get('train', vector_length=10)
    _, int_test_features, test_features, still_test_features, _, _, test_labels = data.get('test', vector_length=10)
    print('train data', train_features.shape, train_labels.shape)
    print('test data', test_features.shape, test_labels.shape)
    print(test_features[79,:], test_labels[79])

    train_features = preprocessing.scale(train_features)
    test_features = preprocessing.scale(test_features)

    still_train_features = train_features.reshape(-1, 1, 3)
    still_test_features = test_features.reshape(-1, 1, 3)
