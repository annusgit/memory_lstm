...
# for the lstm input, you must tell it the sequence length, and the dimension of each vector in the sequence
# the None is the batch size!
x = tf.placeholder("float", [None, n_steps, n_input])
# and y will give its prediction in one hot format
y_ = tf.placeholder("float", [None, n_classes])

# LSTM layer, basically the state of the lstm is the cell state accompanied by the hidden state
# so the sum is equal to 2*hidden_size
# 2 x n_hidden = state_size = (hidden state + cell state)
# again, None is the batch size
istate = tf.placeholder("float", [None, 2*n_hidden])
# so one weight is for the cell state and the other one for the hidden state
weights = {
